...
#if !defined(NOALIGN) && !defined(__APPLE__)
...
void EXPORT x64Syscall_linux(x64emu_t *emu)
{
    ...
    if(s<cnt && syscallwrap[s].nats) {
        ...
        switch(syscallwrap[s].nbpars) {
            ...
            case 1: 
                ...
                break;
            case 2: 
                if(s==33) {if(log) snprintf(buff2, 127, " [sys_access(\"%s\", %lld)]", (char*)R_RDI, (long long)R_RSI);}; 
            ...
            case 5: 
                if(s==165) {if(log) snprintf(buff2, 127, " [sys_mount(%s, %s, %s, 0x%llx, %s]", (char*)R_RDI, (char*)R_RSI, (char*)R_RDX, (unsigned long long)R_R10, R_R8?(char*)R_R8:"(nil)");}; 
    ...
    switch (s) {
        ...
        case 201: // sys_time
            ...
            break;
        #endif
        #if !defined(__NR_epoll_wait) || !defined(NOALIGN) || defined(__APPLE__)
        case 232:
            S_RAX = -ENOSYS;
            break;
        #endif
        #if !defined(__NR_epoll_ctl) || !defined(NOALIGN) || defined(__APPLE__)
        case 233:
            S_RAX = -ENOSYS;
            break;
        #endif
        ...
        case 282:   // sys_signalfd
            ...
            break;
        #if !defined(_NR_eventfd) && !defined(__APPLE__)
        ...
        case 439:
            ...
            break;
        #endif
        #if (!defined(__NR_epoll_pwait2) || !defined(NOALIGN)) && !defined(__APPLE__)
    ...
    if(log) {
        if(BOX64ENV(rolling_log))
            snprintf(buffret, 127, "0x%llx%s", (unsigned long long)R_RAX, buff2);
        else
            printf_log_prefix(0, LOG_NONE, "=> 0x%llx%s\n", (unsigned long long)R_RAX, buff2);
    }
}
