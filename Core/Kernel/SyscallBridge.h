...
#ifdef __APPLE__
#include <dlfcn.h>
inline void _XeniOS_JIT_Toggle(int protect) {
    typedef void (*jit_func)(int);
    static jit_func f = (jit_func)dlsym(RTLD_DEFAULT, "pthread_jit_write_protect_np");
    if (f) f(protect);
}
#define JIT_BEGIN_WRITE()  _XeniOS_JIT_Toggle(0)
#define JIT_END_WRITE()    _XeniOS_JIT_Toggle(1)
